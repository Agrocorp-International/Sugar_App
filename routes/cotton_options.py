...
from models.cotton import (
    CottonTradePosition, CottonMarketPrice,
    CottonSimLoadedFuture, CottonSimLoadedOption, CottonSimStack,
)
from routes.cotton_positions import compute_maps
from routes.cotton_prices import _build_expiry_map, _OPTION_RE
...
from routes.cotton_info import PARSED_CT_FUTURES
from services.exchange_calendar import workday, HOLIDAY_DATES
from routes.cotton_positions import build_contract_key, LOT_MULTIPLIERS_BY_PREFIX as _MULTIPLIERS
from services.iv_utils import calculate_scenario_iv
from services.request_cache import get_all_cotton_positions

cotton_options_bp = Blueprint("cotton_options", __name__)

# ICE Cotton #2 lot multiplier: 50,000 lbs * cents/lb / 100 = 500 USD per lot per cent.
_COTTON_MULT = 500
...
def _spec_options_query():
    ...
    The cotton-side "Alpha" book label is mapped to SF Book__c='Spec' by
    routes/cotton_positions.resolve_cotton_book_to_sf, so this filter selects
    the cotton spec book.
    """
    all_pos = get_all_cotton_positions()
    return [
        p for p in all_pos
...
def _compute_greeks(legs_pos, price_map, as_of, source='sett1'):
    ...
    for p in legs_pos:
        ...
        result[p.sf_id] = {
            ...
            'vega':  vega  * net_lots * _COTTON_MULT / 100,
            'theta': theta * net_lots * _COTTON_MULT,
        }
...
@cotton_options_bp.route("/options")
def index():
    ...
    # CT futures reference month list (no spaces, e.g. "CTH26")
    ct_futures = [
        {'code': f['contract'].replace(' ', ''), 'label': f['contract'], 'expiry': f['expiry']}
        for f in PARSED_CT_FUTURES
        if f['expiry'] is not None
    ]
    # Keep only non-expired contracts
    ct_futures = [f for f in ct_futures if f['expiry'] >= as_of_date]
    # Default: front month = smallest expiry; fallback to last
    default_ref_month = (
        min(ct_futures, key=lambda f: f['expiry'])['code']
        if ct_futures else ''
    )
    ...
    for f in ct_futures:
        all_keys.add(f['code'])
    price_map = {mp.contract: mp for mp in CottonMarketPrice.query.filter(
        CottonMarketPrice.contract.in_(all_keys)).all()}

    ref_month_mp = price_map.get(ref_month)
    ref_month_settlement = resolve_price(ref_month_mp, price_source)

    # Settlement lookup for all CT futures (for dynamic ref-month switching in JS)
    ct_sett_map = {}
    for f in ct_futures:
        mp = price_map.get(f['code'])
        ct_sett_map[f['code']] = resolve_price(mp, price_source)
    ...
    return render_template("cotton/options.html",
                           trade_ids=trade_ids,
                           selected=selected,
...
def _all_unrealised():
    ...
    return [
        p for p in get_all_cotton_positions()
        if p.data.get('Realised__c') == 'Unrealised'
    ]


@cotton_options_bp.route("/options/sim/load-positions", methods=["POST"])
def sim_load_positions():
    ...
    for p in all_pos:
        ...
        all_keys.add(underlying)
    price_map = {mp.contract: mp for mp in CottonMarketPrice.query.filter(
        CottonMarketPrice.contract.in_(all_keys)).all()} if all_keys else {}
    ...
    for p in all_pos:
        ...
        leg_greeks = greeks_map.get(p.sf_id, {})
        leg_delta_from_mp = delta_map.get(p.sf_id)  # CottonMarketPrice.delta (same as Payoff)
        ...
        if not m:
            ...
            if key not in opt_groups:
                ...
                opt_groups[key] = {
                    ...
                    'position': 0,  # delta from CottonMarketPrice (same as Payoff)
                }
    ...
    CottonSimLoadedFuture.query.delete()
    CottonSimLoadedOption.query.delete()
    db.session.commit()
    ...
    for g in opt_groups.values():
        ...
        # Portfolio delta: from CottonMarketPrice.delta (same as Payoff)
...
@cotton_options_bp.route("/options/sim/futures/<int:fid>", methods=["PUT"])
def sim_update_future(fid):
    row = CottonSimLoadedFuture.query.get_or_404(fid)
    body = request.get_json(force=True)
...
@cotton_options_bp.route("/options/sim/options/<int:oid>", methods=["PUT"])
def sim_update_option(oid):
    row = CottonSimLoadedOption.query.get_or_404(oid)
    body = request.get_json(force=True)
    ...
    if row.iv and row.iv > 0 and row.underlying_price and row.expiry_date:
        ...
        if T > 0:
            try:
                ...
                row.vega = _black76_vega(row.underlying_price, row.strike, T, row.r, row.iv) * row.net_lots * _COTTON_MULT / 100
                row.theta = _black76_theta(row.underlying_price, row.strike, T, row.r, row.iv, is_call) * row.net_lots * _COTTON_MULT
            except (ValueError, ZeroDivisionError):
                pass
    db.session.commit()
    return jsonify({
...
@cotton_options_bp.route("/options/sim/futures/<int:fid>", methods=["DELETE"])
...
@cotton_options_bp.route("/options/sim/options/<int:oid>", methods=["DELETE"])
...
@cotton_options_bp.route("/options/sim/reset-limits", methods=["POST"])
def sim_reset_limits():
    for f in CottonSimLoadedFuture.query.all():
        f.lower_limit = round(f.settlement * 0.8, 2)
        f.upper_limit = round(f.settlement * 1.2, 2)
    for o in CottonSimLoadedOption.query.all():
...
@cotton_options_bp.route("/options/sim/run", methods=["POST"])
def sim_run():
    from services.price_source import get_price_source, resolve_price
    price_source = get_price_source()
    body = request.get_json(force=True)
    ...
    futures = CottonSimLoadedFuture.query.all()
    options = CottonSimLoadedOption.query.all()

    # Find the anchor future for the x-axis
    # If not in loaded positions, look up CottonMarketPrice and create a virtual anchor
    ...
    if anchor:
        ...
        mp = CottonMarketPrice.query.filter_by(contract=commodity_code).first()
    ...
    if x_axis == 'Price':
        ...
        # Round to tick size (0.01 for cotton, same as sugar)
        tick_size = 0.01
...
@cotton_options_bp.route("/options/sim/theta-decay", methods=["POST"])
def sim_theta_decay():
    options = CottonSimLoadedOption.query.all()
    if not options:
...
@cotton_options_bp.route("/options/sim/stacks", methods=["GET"])
def sim_list_stacks():
    stacks = CottonSimStack.query.order_by(CottonSimStack.created_at.desc()).all()
    return jsonify([{
...
@cotton_options_bp.route("/options/sim/stacks", methods=["POST"])
def sim_save_stack():
    body = request.get_json(force=True)
    stack = CottonSimStack(
...
@cotton_options_bp.route("/options/sim/stacks/<int:sid>", methods=["DELETE"])
def sim_delete_stack(sid):
    row = CottonSimStack.query.get_or_404(sid)
    db.session.delete(row)
    db.session.commit()
    return jsonify({'ok': True})
