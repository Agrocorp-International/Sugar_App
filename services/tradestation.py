...
# ── Option expiry lookup (for /info page) ────────────────────────────────────

# Candidate strikes to probe per product root.
# We only need one valid quote per series to read LastTradingDate.
...
def fetch_option_expiries(series_codes):
    """
    Fetch LastTradingDate from TradeStation for a list of option series.

    Args:
        series_codes: e.g. ["SB F26", "SB G26", "SB H26", ...]
                      Format: "<root> <month_code><YY>"

    Returns:
        {series_code: datetime.date | None}
        Empty dict on auth failure. Per-series None on probe failure.
    """
    if not series_codes:
        return {}
    ...
    try:
        ...
        _log.warning("fetch_option_expiries: auth failed — %s", e)
        return {}
    headers = {"Authorization": f"Bearer {token}"}

    # 2. Build candidate symbols per series
    # series "SB G26" → base "SBG26" → candidates "SBG26C1400", "SBG26C1600", ...
    # Map: ts_symbol → series_code (for reverse lookup after batch fetch)
    ts_to_series = {}       # {ts_symbol: series_code}
    series_candidates = {}  # {series_code: [ts_symbol, ...]}

    for series in unique:
        parts = series.split()
        if len(parts) != 2:
            _log.warning("fetch_option_expiries: bad series code %r, skipped", series)
            continue
        root = parts[0]           # "SB" or "SW"
        month_year = parts[1]     # "G26"
        base = root + month_year  # "SBG26"
        strikes = _EXPIRY_PROBE_STRIKES.get(root, _EXPIRY_PROBE_STRIKES["SB"])
        candidates = []
        for strike in strikes:
            internal = base + strike          # "SBG26C1800"
            ts_sym = to_tradestation_symbol(internal)
            if ts_sym:
                candidates.append(ts_sym)
                ts_to_series[ts_sym] = series
        series_candidates[series] = candidates

    if not ts_to_series:
        return {}

    # 3. Batch-fetch quotes for ALL candidate symbols at once
    try:
        quote_map = _batch_quotes(list(ts_to_series.keys()), headers)
    except RuntimeError as e:
        _log.warning("fetch_option_expiries: quote fetch failed — %s", e)
        return {}

    # 4. Extract LastTradingDate — first valid hit per series wins
    result = {}
    for series in unique:
        candidates = series_candidates.get(series, [])
        resolved = None
        for ts_sym in candidates:
            q = quote_map.get(ts_sym)
            if q is None:
                continue
            ...
            try:
                ...
                _log.warning(
                    "fetch_option_expiries: malformed LastTradingDate %r for %s — %s",
                    ltd, ts_sym, e,
                )
        result[series] = resolved
    ...
    _log.info(
        "fetch_option_expiries: %d/%d series resolved. Fallback: %s",
